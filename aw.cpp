...
#include <tuple>
#include <memory>
#include <cmath>#include <windows.h>
#include <string>
#include <vector>
#include <filesystem>
#include <fstream>
...
namespace h{
        namespace constGlobalData{
            ...
            constexpr auto SETTINGFILE="setting.ini";
        };
        namespace cast{
        template <class T>
...
BOOL CALLBACK enumChildSave(HWND hwnd,LPARAM lp){
    ...
    h::INI(h::constGlobalData::SETTINGFILE)
...
BOOL CALLBACK enumChildLoad(HWND hwnd,LPARAM lp){
    auto rect=h::StrToInt(h::INI(h::constGlobalData::SETTINGFILE).getData<h::INIT::keyT>("POS",h::getWindowStr(hwnd)),4);
...
LRESULT CALLBACK WndProc(HWND hwnd,UINT msg,WPARAM wp,LPARAM lp){
    ...
    switch(msg){
        case WM_DESTROY:
            ...
            h::INI(h::constGlobalData::SETTINGFILE).editValue("STRING","wordList",str)
        ...
        case WM_CREATE:
            ...
            if(!std::filesystem::exists(h::constGlobalData::SETTINGFILE)){
                std::ofstream(h::constGlobalData::SETTINGFILE);
            }
            for(auto word:h::split(h::INI(h::constGlobalData::SETTINGFILE).getData<h::INIT::keyT>("STRING","wordList"),"[CELEND]")){
                SendMessage(list,LB_ADDSTRING,0,(LPARAM)word.c_str());
            }
            {
                auto re=h::StrToInt(h::INI(h::constGlobalData::SETTINGFILE).getData<h::INIT::keyT>("POS","Mode"),4);
